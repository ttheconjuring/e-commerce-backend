...
This service is the "aggregate root" for the `Order` entity and the main **entry point** for the entire system. Its
primary role is to handle the creation and final state of an order.
...
* Initiates the entire saga by persisting an `Order` and publishing an `OrderCreatedEvent` using the Transactional
  Outbox pattern.
* Listens for final commands from the orchestrator (e.g., `CompleteOrderCommand`) to update the order's status to a
...
* **Create Order:** `POST /api/orders/create`
    * **Body:** `CreateOrderRequest` JSON object (look at `json/initial.txt`).
